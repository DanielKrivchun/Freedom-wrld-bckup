...
public class BezierPointEditor : Editor {
	...
	void OnSceneGUI()
	{
		...
		var fmh_96_64_638539770321902504 = point.transform.rotation; Vector3 newPosition = Handles.FreeMoveHandle(point.position, HandleUtility.GetHandleSize(point.position)*0.2f, Vector3.zero, Handles.CubeHandleCap);
	...
	private static void HandleConnected(BezierPoint p){
		...
		var fmh_111_64_638539770321928551 = p.transform.rotation; Vector3 newGlobal1 = Handles.FreeMoveHandle(p.globalHandle1, HandleUtility.GetHandleSize(p.globalHandle1)*0.15f, Vector3.zero, Handles.SphereHandleCap);
		...
		var fmh_119_64_638539770321931574 = p.transform.rotation; Vector3 newGlobal2 = Handles.FreeMoveHandle(p.globalHandle2, HandleUtility.GetHandleSize(p.globalHandle2)*0.15f, Vector3.zero, Handles.SphereHandleCap);
	...
	private static void HandleBroken(BezierPoint p){
		...
		var fmh_131_64_638539770321934859 = Quaternion.identity; Vector3 newGlobal1 = Handles.FreeMoveHandle(p.globalHandle1, HandleUtility.GetHandleSize(p.globalHandle1)*0.15f, Vector3.zero, Handles.SphereHandleCap);
		var fmh_132_64_638539770321937633 = Quaternion.identity; Vector3 newGlobal2 = Handles.FreeMoveHandle(p.globalHandle2, HandleUtility.GetHandleSize(p.globalHandle2)*0.15f, Vector3.zero, Handles.SphereHandleCap);
